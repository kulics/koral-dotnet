        ...
        public override object VisitTypeFunction(TypeFunctionContext context)
        {
            ...
            if (context.t.Type == Right_Arrow)
            {
                ...
                else
                {
                    ...
                    {
                        var @out = (new System.Text.StringBuilder("(").Append(@out).Append(")")).to_str();
                    }
                    if (@in.Length == 0)
                    {
                        var obj = (new System.Text.StringBuilder("Func<").Append(@out).Append(">")).to_str();
                    }
                    else
                    {
                        var obj = (new System.Text.StringBuilder("Func<").Append(@in).Append(", ").Append(@out).Append(">")).to_str();
                    }
                }
            }
            else
            {
                if (@out.Length == 0)
                {
                    if (@in.Length == 0)
                    {
                        var obj = (new System.Text.StringBuilder("Func<").Append(Task).Append(">")).to_str();
                    }
                    else
                    {
                        var obj = (new System.Text.StringBuilder("Func<").Append(@in).Append(", ").Append(Task).Append(">")).to_str();
                    }
                }
                else
                {
                    if (@in.Length == 0)
                    {
                        var obj = (new System.Text.StringBuilder("Func<").Append(Task).Append("<").Append(@out).Append(">>")).to_str();
                    }
                    else
                    {
                        var obj = (new System.Text.StringBuilder("Func<").Append(@in).Append(", ").Append(Task).Append("<").Append(@out).Append(">>")).to_str();
                    }
                }
            }
            return (obj);
